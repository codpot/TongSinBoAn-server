...
exports.read = function (group_idx, callback) {
  db.query("SELECT m.idx, m.name, m.group_idx, g.name as group_name, m.level, m.token, m.token_valid, m.enabled, m.regdate FROM `member` AS m LEFT JOIN `group` AS g ON m.group_idx = g.idx WHERE m.group_idx=?;", [group_idx], function (error, results) {
    if (!error) {
...
exports.read_idx = function (member_idx, callback) {
  db.query("SELECT m.idx, m.name, m.group_idx, g.name as group_name, m.level, m.token, m.token_valid, m.enabled, m.regdate FROM `member` AS m LEFT JOIN `group` AS g ON m.group_idx = g.idx WHERE m.idx=?;", [member_idx], function (error, results) {
...
exports.read_login = function (userid, passwd, callback) {
  db.query("SELECT m.idx, m.name, m.group_idx, g.name as group_name, m.level, m.token, m.token_valid, m.enabled, m.regdate FROM `member` AS m LEFT JOIN `group` AS g ON m.group_idx = g.idx WHERE m.userid = ? AND m.passwd = ?;", [userid, passwd], function (error, results) {
    if (!error && results.length === 1) {
      callback(true, results);
    } else {
      callback(false);
